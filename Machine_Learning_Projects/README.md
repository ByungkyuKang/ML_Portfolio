...
🛠️ Key Steps:
    - Implemented 5-fold cross-validation using cross_validate() with RandomForestClassifier (random_state=12).
    - Calculated and compared key metrics:
        Accuracy: 0.80 ± 0.03
        F1-score: 0.74 ± 0.05
        ROC-AUC: 0.86 ± 0.03
    - Performed a manual check using StratifiedKFold to verify fold consistency, resulting in a mean accuracy of 0.79.

✅ Result:
    - Model shows stable performance across folds with low variance — indicating good generalization.
    - ROC-AUC (0.86) suggests the model effectively distinguishes survivors from non-survivors.
